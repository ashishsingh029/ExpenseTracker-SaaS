...
export const last30DaysIncomeAnalyticsService = async userId => {
    ...
    const totalIncomeInLast30Days = last30DaysIncomeTransactions.reduce((sum, transaction) => sum + transaction.amount, 0)
...
export const last30DaysExpenseAnalyticsService = async userId => {
    ...
    const totalExpenseInLast30Days = last30DaysExpenseTransactions.reduce((sum, transaction) => sum + transaction.amount, 0)
...
export const getFiveMostRecentTransactionsService = async userId => {
    ...
    const fiveMostRecentTransactions = [
        ...
        ...recentExpenses.map(transaction => ({ ...transaction, type: "expense" }))
    ];
    
    fiveMostRecentTransactions.sort((a, b) => new Date(b.date) - new Date(a.date));
